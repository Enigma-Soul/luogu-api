...
def add_training_problem(tid, pids=None):
    """向题单添加题目"""
    if pids is None:
        pids = input("输入题目ID (逗号分隔): ").strip().split(",")
...
def edit_training_problems(tid, pids=None):
    """重排题单题目"""
    if pids is None:
        pids = input("输入题目ID (逗号分隔，按顺序): ").strip().split(",")
...
if __name__ == "__main__":
    ...
    tid = input("  测试题单ID (默认1): ").strip() or "1"
